...
export const FormSong = ({ action, song }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const response =
      action === "Update"
        ? await updatesong(formDataToSend)
        : await NewSong(formDataToSend);
    ...
    if (file) {
      ...
      if (file.type === "audio/mpeg") {
        ...
        audio.onloadedmetadata = () => {
          ...
          const kbps = ((file.size * 8) / (durationInSeconds * 1000)).toFixed(
            2
          );
        ...
        alert(
          "Por favor, sube un archivo MP3 para la canción o una imagen para la img."
        );
      }
    }
  };
